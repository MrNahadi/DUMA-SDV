...
**Be token-lean.** Grep for the answer before reading; read only the matching sections (`grep -n`, then a focused range), never whole files. At most two web fetches per question. Your whole reply stays under 250 words.
...
2. `specs/tech-stack.md`, `docs/glossary.md`, `docs/design-rules.md`, `docs/adr/` (grep titles first: `grep -l <topic> docs/adr/*`), `questions/answered/`.
