...
- Chart series use dedicated data colours, separate from UI accent and status colours. The Charge curve uses `--data-ac` for AC (`#0072B2`, blue) and `--data-dc` for DC (`#C65300`, orange). Both are legible against the white chart surface; series must also be identified in words. Add new data series colours here before using them in another chart. The Cycle chart uses `--data-speed` for speed (`#0072B2`, blue), `--data-target` for target speed (`#5A5A5A`, grey, dashed), `--data-power` for battery power (`#C65300`, orange) and `--data-soc` for SOC (`#007A5E`, bluish green); each series is named in words and the chart sits in the Cycles view, never beside the Charge curve. The PDF vehicle report (ADR 0021) reuses `--data-speed`, `--data-power` and `--data-soc` for its speed, battery power and SOC charts, and for its temperature chart uses `--data-power` for the pack, `--data-speed` for the motor and `--data-soc` for the inverter, each named in a legend.
