...
- [ ] **09 · Stage visual refresh.** Replace the blocky procedural car with a smooth stylised one in the style of slowroads.io: a lofted, smooth-shaded body with wheel arches, one-piece tinted glasshouse, five-spoke wheels, full-width light strips, clearcoat paint and soft procedural studio reflections. Replace the faint rolling stripes with a road strip (dashed centre line, edge lines, roadside posts, fading into fog) whose motion stays readable up to top speed. Part names, internals, ghost mode, budgets and 60 fps still hold. Playwright saves stage screenshots for human review. Decisions in ADR 0014.
- [ ] **10 · OTA software update** [6]. Software view with ECU versions; download → verify → install → reboot flow with confirmation; the update visibly changes behaviour (e.g. unlocks a mode or raises regen strength).
- [ ] **11 · Guided demo mode** [5]. One button plays each scenario with captions. "Start here" onboarding for first-time judges. Includes one e2e test per scenario, which closes the DoD scenario check.
- [ ] **12 · Offline and deployment** [11]. PWA / service worker, documented offline check, public static hosting.
