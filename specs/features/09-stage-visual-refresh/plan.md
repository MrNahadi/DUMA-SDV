...
Source: roadmap phase 09, ADR 0014 (supersedes only the styling in ADR 0002), brief §4 (60 fps on integrated graphics), DESIGN-RULES §3 and §6.
...
4. **Tokens before colours.** Add road surface, marking and post tokens to DESIGN-RULES §3, `tokens.css` and `tokens.ts`, with a contrast test of at least 1.5:1. Add one line to DESIGN-RULES §6 allowing road motion that tracks travel.
...
- `src/ui/tokens.css`, `src/ui/tokens.ts`, `DESIGN-RULES.md` §3 and §6
