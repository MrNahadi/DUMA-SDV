...
4. **Energy view.** Route the Energy nav item (`activity`) to a panel whose subject is the live power-flow diagram (hand-written SVG, no new packages). Animated dashes follow power sign and size (DESIGN-RULES §6), with a temperature readout and coolant loop state alongside.
