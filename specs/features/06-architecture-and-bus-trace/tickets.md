...
Acceptance:
- [x] `npm run e2e` passes: every spec's console-error check currently gets two "Failed to load resource: 403 (Forbidden)" errors (14 specs fail, 5 pass)
- [x] `npm test` passes as a full suite: `startup.test.ts` determinism (5 s), `dc-reference.test.ts` (30 s) and `energy.test.ts` (60 s) time out in the full run but pass alone
